...
colom_for_arrays =  size ( x_pose , 2 ) ; 
force_in_R=zeros(2, colom_for_arrays);
force_norm=zeros(2, colom_for_arrays);
orgin_arrow=zeros(2,colom_for_arrays);
end_arrow=zeros(2, colom_for_arrays);
...
% we have the quar data from the bag file 
 ...
 q = [owdata(j) oxdata(j) oydata(j) ozdata(j)]; % j = the index of the position with the highest force
    ...
    force_norm(:,:) = force_in_R(:,:)/norm(force_in_R(:,:)); % ??? 
    ...
    end_arrow(:,:) = orgin_arrow(:,:) + force_in_R(:,:); % the end of the arraw is the force in robot frame 
    end_arrow(:,:) = end_arrow(:,:)/10;
